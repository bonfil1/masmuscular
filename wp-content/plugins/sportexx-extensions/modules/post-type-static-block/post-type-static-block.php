	...
	static function init_static_blocks() {
		if (function_exists('register_post_type')) {
			register_post_type('static_block',
				array(
					'labels' => array(
						'name' 					=>	_x('Static Content Blocks', 'post type general name', 'sportexx-extensions'),
						'singular_name' 		=>	_x('Static Block', 'post type singular name', 'sportexx-extensions'),
						'add_new' 				=>	_x('Add New', 'block', 'sportexx-extensions'),
						'add_new_item' 			=>	__('Add New Block', 'sportexx-extensions'),
						'edit_item' 			=>	__('Edit Block', 'sportexx-extensions'),
						'new_item' 				=>	__('New Block', 'sportexx-extensions'),
						'all_items' 			=>	__('Static Blocks', 'sportexx-extensions'),
						'view_item' 			=>	__('View Block', 'sportexx-extensions'),
						'search_items' 			=>	__('Search', 'sportexx-extensions'),
						'not_found' 			=>	__('No blocks found', 'sportexx-extensions'),
						'not_found_in_trash' 	=>	__('No blocks found in Trash', 'sportexx-extensions'), 
						'parent_item_colon' 	=> '',
...
$meta_box_static_blocks = array(
	...
	'title' 	=>  __('Content Options', 'sportexx-extensions'),
	'page' 		=> 'static_block',
	'context' 	=> 'side',
	'priority' 	=> 'default',
	'fields' 	=> array(
    	array(
    	   'name' 		=> __('Content Filters', 'sportexx-extensions'),
    	   'desc' 		=> __('Apply all WP content filters? This will include plugin added filters.', 'sportexx-extensions'),
    	   'id' 		=> 'content_filters',
    	   'type' 		=> 'radio',
    	   'std' 		=> '',
    	   'options'	=> array(
    	   		'default'	=> __('Defaults (recommended)', 'sportexx-extensions'),
    	   		'all' 		=> __('All Content Filters', 'sportexx-extensions')
    	   	)
    	),
    	array(
    	   'name' 		=> __('Auto Paragraphs', 'sportexx-extensions'),
    	   'desc' 		=> __('Add &lt;p&gt; and &lt;br&gt; tags automatically.<br>(disabling may fix layout issues)', 'sportexx-extensions'),
    	   'id' 		=> 'wpautop',
    	   'type' 		=> 'radio',
    	   'std' 		=> '',
    	   'options' 	=> array(
    	   		'on' 	=> __('On', 'sportexx-extensions'),
    	   		'off' 	=> __('Off', 'sportexx-extensions')
    	   	)
    	)
   	)
);
...
function sportexx_add_box_static_blocks() {
	global $meta_box_static_blocks;
	
	add_meta_box(
		$meta_box_static_blocks['id'], 
		$meta_box_static_blocks['title'], 
		'sportexx_show_box_static_blocks', 
...
add_action('admin_menu', 'sportexx_add_box_static_blocks');
...
function sportexx_show_box_static_blocks() {
	global $meta_box_static_blocks, $post;
 	
	// Use nonce for verification
	echo '<input type="hidden" name="sportexx_meta_box_nonce" value="', wp_create_nonce( basename(__FILE__) ), '" />';
...
add_action('save_post', 'sportexx_save_data_static_blocks');
...
function sportexx_save_data_static_blocks($post_id) {
	global $meta_box_static_blocks;
 
	// verify nonce
	if ( !isset($_POST['sportexx_meta_box_nonce']) || !wp_verify_nonce($_POST['sportexx_meta_box_nonce'], basename(__FILE__))) {
		return $post_id;
	}
