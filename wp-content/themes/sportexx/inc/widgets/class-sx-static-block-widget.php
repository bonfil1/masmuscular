...
class SX_Static_Block_Widget extends WC_Widget {

	public function __construct() {

		$static_block_options = $this->get_static_blocks_options();

		$this->widget_cssclass    = 'sportexx widget-static-block';
		$this->widget_description = __( 'Display a static block.', 'sportexx' );
		$this->widget_id          = 'sx_static_block';
		$this->widget_name        = __( 'Static Block', 'sportexx' );
		$this->settings           = array(
			'static_block_id' 	=> array(
				'type'  		=> 'select',
				'std'   		=> '',
				'label' 		=> __( 'Select a Static Block', 'sportexx' ),
	...
	private function get_static_blocks_options() {
		...
		$options[0] = __( 'Choose a static block', 'sportexx' );
